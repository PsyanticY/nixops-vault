...
class VaultKVSecretEngineState(
    nixops.resources.DiffEngineResourceState[VaultKVSecretEngineDefinition]
):
    """State of a vault kv secret engine."""

    definition_type = VaultKVSecretEngineDefinition
    ...
    def __init__(self, depl, name, id):
        ...
        self.handle_create_engine = Handler(
            [
                "name",
                "vaultAddress",
                "type",
        ...
        self.handle_update_engine = Handler(
            [
                "maxLeaseTtl",
                "listingVisibility",
    ...
    def realize_create_secrets(self, allow_recreate):
        ...
        for i in config.secrets:
            data = {"data": {d.key: d.value for d in i.data}}
            metadata = {"max_versions": i.maxVersions}
            r = nixops_vault.vault_common.vault_post(
                self._state["vaultToken"],
                self._state["vaultAddress"],
                self._state["name"] + "/data/" + i.path,
                data,
                "secret",
            )
            ...
            r = nixops_vault.vault_common.vault_post(
                self._state["vaultToken"],
                self._state["vaultAddress"],
                self._state["name"] + "/metadata/" + i.path,
                metadata,
                "secret",
            )
            if r.status_code != 204:
    ...
    def _check(self):
        ...
        if r.status_code == 400:
            self.warn(
                "kv secret engine '{0}' was deleted from outside nixops,"
